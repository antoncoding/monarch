...
11. **Locale-safe decimal inputs**: transaction-critical amount/slippage inputs (including compact inline edit controls) must accept both `,` and `.`, preserve transient edit states (e.g. `''`, `.`) during typing, and only normalize/clamp on commit (`blur`/submit) so delete-and-retype flows never lock users into stale values.
