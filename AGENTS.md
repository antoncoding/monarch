...
19. **Swap preview parity and route-control minimalism**: swap-backed leverage/deleverage UIs must display the executable rate and slippage from the same quote/config used by the tx builder, and route selection must expose a single visible control instead of duplicated badge + switcher affordances.
20. **Same-pair swap leverage normalization**: when a leverage flow starts from the market loan asset and the executed swap pair is the same `loan -> collateral` leg used for the flash path, quote/preview/tx-building must treat user input plus flash-borrowed loan as one combined sell amount; do not model a phantom pre-conversion leg that is not actually executed on-chain.
