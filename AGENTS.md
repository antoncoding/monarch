...
44. **Primary-empty fallback integrity**: when a primary indexer/provider backs market participants, user-position discovery, or similar source-discovery reads, an empty result from the primary must not be treated as authoritative absence if fallback providers still exist. Empty primary pages may reflect indexer lag or partial coverage; continue to the next provider unless the primary explicitly proves completeness for that scope.
