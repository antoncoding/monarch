...
61. **Vault adapter history-source integrity**: empty Envio user-transaction results for recognized `MorphoMarketV1Adapter` / `MorphoMarketV1AdapterV2` addresses are not authoritative. Before returning empty adapter history in vault or adapter-position views, verify whether the queried address is a recognized market adapter and fall back to Morpho API (and then subgraph, if applicable) so adapter rebalance history is not dropped by the primary-empty short circuit.
62. **Market historical-rate window integrity**: market-list 24h/7d/30d supply/borrow APY enrichments derived from on-chain share growth must use Morpho share math with virtual assets/shares and must fail closed when the compared snapshots do not actually bracket the requested window, including stale `lastUpdate` on either boundary. Do not annualize stale accrual over a shorter labeled window.
