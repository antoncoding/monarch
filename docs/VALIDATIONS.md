...
- Position cold loads must batch known market IDs per chain, including token metadata resolution, rather than issue one market request per position. Verify request counts, output parity, partial-result fallback, and chain isolation with a many-position account. Live comparisons must match the deployed API/RPC and authentication configuration without exposing credentials.
