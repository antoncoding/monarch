...
- Position refreshes must distinguish failed RPC reads from confirmed zero balances. Verify transport and partial multicall failures preserve successful cached positions, expose a retryable error, and never render a false empty portfolio; optional oracle failures must not discard balances.
