...
import { MarketDebtIndicator, MarketAssetIndicator, MarketOracleIndicator } from 'app/markets/components/RiskIndicator';
...
export function RiskSelection() {
  ...
  const filteredMarkets = useMemo(() => {
    ...
    return selectedToken.markets.filter(market => {
      ...
      if (selectedCollaterals.length > 0) {
        const tokenKey = `${market.collateralAsset.address.toLowerCase()}-${market.morphoBlue.chain.id}`;
  ...
  const handleNext = () => {
    if (selectedMarkets.size > 0) {
      const selectedMarketsArray = Array.from(selectedMarkets).map(key => 
        filteredMarkets.find(m => m.uniqueKey === key)
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Input Section */}
      <div className="mt-6 flex gap-4 border-b border-gray-700/50 pb-6">
              ...
              {filteredMarkets.map((market) => {
                ...
                return (
                  <tr
                    ...
                    className={`cursor-pointer border-b border-gray-200 transition-all duration-200 ease-in-out hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800 ${
                  ...
                  >
                    <td className="whitespace-nowrap px-4 py-2">
                      <div className="flex items-center gap-2">
                        {collateralToken?.img && (
                          <div className={`overflow-hidden rounded-full transition-all duration-200 ease-in-out ${isSelected ? 'ring-2 ring-primary ring-offset-2 dark:ring-offset-gray-900' : ''}`}>
                            <Image
