...
export const processTransactionData = (
  ...
  const initializeAssetTracking = (assetAddress: string, chainId: SupportedNetworks) => {
    ...
    if (!assetMetrics[assetKey]) {
      ...
      console.log(
        `Initializing asset tracking for ${assetAddress} (${symbol}) on chain ${chainId}`,
      );
  ...
  transactions.forEach((tx) => {
    ...
    if (tx.supplies && Array.isArray(tx.supplies)) {
      tx.supplies.forEach((supply) => {
        ...
        assetMetrics[assetKey].supplyVolume = (
          BigInt(assetMetrics[assetKey].supplyVolume) + supplyVolumeBigInt
        ).toString();
        assetMetrics[assetKey].totalVolume = (
          BigInt(assetMetrics[assetKey].totalVolume) + supplyVolumeBigInt
        ).toString();
    ...
    if (tx.withdrawals && Array.isArray(tx.withdrawals)) {
      tx.withdrawals.forEach((withdrawal) => {
        ...
        assetMetrics[assetKey].withdrawVolume = (
          BigInt(assetMetrics[assetKey].withdrawVolume) + withdrawVolumeBigInt
        ).toString();
        assetMetrics[assetKey].totalVolume = (
          BigInt(assetMetrics[assetKey].totalVolume) + withdrawVolumeBigInt
        ).toString();
    ...
    if (tx.supplies?.length === 0 && tx.withdrawals?.length === 0 && tx.market) {
      ...
      if (typeof tx.market === 'string') {
        ...
        assetAddress = marketObj.loan
          ? marketObj.loan.toLowerCase()
          : marketObj.id
          ? marketObj.id.toLowerCase()
          : undefined;
      }
      ...
      if (assetAddress) {
        ...
        console.log(
          `Added aggregated data for ${assetAddress} from tx ${tx.id} on chain ${chainId}`,
        );
      }
    }
  });
  ...
  console.log(
    `Data processing complete - processed ${suppliesProcessed} supplies and ${withdrawalsProcessed} withdrawals`,
  );
  console.log(`Found ${Object.keys(assetMetrics).length} unique assets`);

  // Filter out assets with no transactions
  const filteredAssets = Object.values(assetMetrics).filter(
    (asset) => asset.supplyCount > 0 || asset.withdrawCount > 0,
  );
