...
export const useMorphoBundlerAuthorization = ({
  ...
  const authorizeBundlerWithSignature = useCallback(async () => {
    if (!account || isBundlerAuthorized === true || !nonce) {
        console.log("Skipping authorizeBundlerWithSignature:", { account, isBundlerAuthorized, nonce });
  ...
  }, [account, isBundlerAuthorized, nonce, chainId, bundlerAddress, signTypedDataAsync, refetchIsBundlerAuthorized, refetchNonce, toast]);
