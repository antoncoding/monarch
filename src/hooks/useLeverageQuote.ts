...
type UseLeverageQuoteParams = {
  ...
  /**
   * Exact extra debt to loop from an existing position. When set, the flash leg
   * uses this debt amount directly instead of deriving it from the added capital.
   */
...
export function useLeverageQuote({
  ...
  const exactLoanInputFlashLoanAssetAmount = isPositionDebtInput
    ? safePositionDebtInputAmount
    : isLoanAssetInput
      ? computeLeveragedExtraAmount(initialCapitalInputAmount, multiplierBps)
      : 0n;
  const exactLoanInputSellAmount = isPositionDebtInput
    ? safePositionDebtInputAmount
    : isLoanAssetInput
      ? initialCapitalInputAmount + exactLoanInputFlashLoanAssetAmount
      : 0n;
  ...
  const quotedInitialCapitalCollateralTokenAmount = useMemo(() => {
    ...
    return 0n;
  }, [route, isSwapLoanAssetInput, isLoanAssetInput, initialCapitalInputAmount, previewDepositCollateralSharesFromLoanAssets]);
  ...
  const {
    data: previewMintableLoanAssets,
  ...
  const totalCollateralTokenAmountAdded = useMemo(() => {
    if (!route) return 0n;
    if (isPositionDebtInput) return initialCapitalCollateralTokenAmount + flashLegCollateralTokenAmount;
    if (route.kind === 'swap') {
