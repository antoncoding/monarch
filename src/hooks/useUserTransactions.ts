...
const useUserTransactions = () => {
  ...
  const fetchTransactions = useCallback(async (filters: TransactionFilters): Promise<TransactionResponse> => {
    setLoading(true);
    setError(null);
        ...
        if (supportsMorphoApi(network)) {
          try {
            ...
            if (!morphoResponse.error) {
              ...
              console.log(`Received ${networkItems.length} items from Morpho API for network ${network}`);
              return {
                items: networkItems,
                pageInfo: {
        ...
        if (!supportsMorphoApi(network) || networkError) {
          try {
            ...
            if (!subgraphResponse.error) {
              ...
              console.log(`Received ${networkItems.length} items from Subgraph for network ${network}`);
              return {
                items: networkItems,
                pageInfo: {
                  count: networkItems.length,
